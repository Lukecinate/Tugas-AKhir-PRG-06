...
- templatess are now be parsed before preview and insertion to make preview consistent with inserted templates content and prevent XSS. #TINY-9244
...
- New `iframe_templates_callback` option in the Media plugin. Patch provided by Namstel. #TINY-8684
...
### Fixed
- Some templates plugin option values were not escaped properly when doing replacement lookups with Regular Expressions. #TINY-7433
...
- Preview and Insert templates dialogs now display the correct content background color when using dark skins. #TINY-8534
...
- Added new plugin commands: `mceEmoticons` (Emoticons), `mceWordCount` (Word Count), and `mcetemplates` (templates). #TINY-7619
...
- Changed the `emoticons`, `wordcount`, `code`, `codesample`, and `templates` plugins to open dialogs using commands. #TINY-7619
...
- Fixed the order of CSS precedence of `content_style` and `content_css` in the `preview` and `templates` plugins. `content_style` now has precedence. #TINY-6529
...
- Added templates support to the `autocompleter` for customizing the autocompleter items. #TINY-6505
...
- Fixed the `templates` plugin previews missing some content styles. #TINY-6115
...
- Fixed an issue where the templates plugin dialog would be indefinitely blocked on a failed templates load. #TINY-2766
...
- Fixed the templates plugin dialog missing the description field. #TINY-3337
...
- Fixed so templates dialog gets resized if it doesn't fit the window viewport.
...
- Added body_class to templates plugin preview. Patch contributed by Milen Petrinski.
...
- Fixed so the templates plugins templatess setting can be a function that gets a callback that can provide templatess.
...
- Fixed bug where the templates plugin wouldn't handle functions as variable replacements.
