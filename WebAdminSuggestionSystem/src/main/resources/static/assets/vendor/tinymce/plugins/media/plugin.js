...
(function () {
    ...
    const register$2 = editor => {
      ...
      registerOption('audio_templates_callback', { processor: 'function' });
      registerOption('video_templates_callback', { processor: 'function' });
      registerOption('iframe_templates_callback', { processor: 'function' });
    ...
    const getAudiotemplatesCallback = option('audio_templates_callback');
    const getVideotemplatesCallback = option('video_templates_callback');
    const getIframetemplatesCallback = option('iframe_templates_callback');
    ...
    const getIframeHtml = (data, iframetemplatesCallback) => {
      if (iframetemplatesCallback) {
        return iframetemplatesCallback(data);
      } else {
    ...
    const getAudioHtml = (data, audiotemplatesCallback) => {
      if (audiotemplatesCallback) {
        return audiotemplatesCallback(data);
      } else {
        return '<audio controls="controls" src="' + data.source + '">' + (data.altsource ? '\n<source src="' + data.altsource + '"' + (data.altsourcemime ? ' type="' + data.altsourcemime + '"' : '') + ' />\n' : '') + '</audio>';
      }
    };
    const getVideoHtml = (data, videotemplatesCallback) => {
      if (videotemplatesCallback) {
        return videotemplatesCallback(data);
      } else {
    ...
    const dataToHtml = (editor, dataIn) => {
      ...
      if (data.embed) {
        ...
        const audiotemplatesCallback = getAudiotemplatesCallback(editor);
        const videotemplatesCallback = getVideotemplatesCallback(editor);
        const iframetemplatesCallback = getIframetemplatesCallback(editor);
        ...
        if (data.type === 'iframe') {
          return getIframeHtml(data, iframetemplatesCallback);
        } else if (data.sourcemime === 'application/x-shockwave-flash') {
          return getFlashHtml(data);
        } else if (data.sourcemime.indexOf('audio') !== -1) {
          return getAudioHtml(data, audiotemplatesCallback);
        } else if (data.type === 'script') {
          return getScriptHtml(data);
        } else {
          return getVideoHtml(data, videotemplatesCallback);
        }
      }
    };
